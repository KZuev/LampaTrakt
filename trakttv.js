(function () {
  ...
  // полноэкранный activity__loader (тот же приём, что в Lampa-Plex — доступ через
  // Lampa.Activity.active().activity.loader). Контент под спиннером приглушается
  // (opacity), а не пропадает целиком; отмена по клику в любое место или Назад
  // на пульте (Apple TV), т.к. у нативного оверлея своей отмены нет.
  ...
  function _traktLoaderShow(onCancel) {
    ...
    try {
      ...
      if (!act || typeof act.loader !== 'function') return false;
      _traktLoaderActivity = act;
      if (act.slide && typeof act.slide.addClass === 'function') act.slide.addClass('trakt-loader-dim');
      act.loader(true);
  ...
  function _traktLoaderHide() {
    try {
      if (_traktLoaderActivity) {
        if (_traktLoaderActivity.slide && typeof _traktLoaderActivity.slide.removeClass === 'function') {
          _traktLoaderActivity.slide.removeClass('trakt-loader-dim');
        }
        _traktLoaderActivity.loader(false);
      }
    } catch (e) {}
