      ...
      onSelect: function (item) {
        if (item.done || item.disable) {
          ...
          var _doReplace = function() {
            if (!desc) {
              try { Lampa.Bell.push({ text: 'Trakt: нет активной страницы — откройте Мой список вручную' }); } catch(_) {}
              return;
            }
            if (!Lampa.Activity || typeof Lampa.Activity.replace !== 'function') {
              try { Lampa.Bell.push({ text: 'Trakt: Activity.replace недоступна' }); } catch(_) {}
              return;
            }
            try {
              ...
              try { Lampa.Bell.push({ text: 'Trakt: ошибка обновления страницы: ' + (e && e.message || String(e)) }); } catch(_) {}
            }
          };
          if (secs.length > 0) {
