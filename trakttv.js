(function () {
  ...
  function watchlistHub(object) {
    ...
    function updateBtn(btn, label, active) {
      ...
      btn.css({ 'background': active ? 'rgba(180,51,154,.18)' : '', 'box-shadow': active ? 'inset 0 0 0 1px rgba(180,51,154,.3)' : '', 'border-bottom': active ? '3px solid #b4339a' : '' });
    }
    ...
    showWatchProgress: function showWatchProgress(data, element) {
      ...
      function buildProgressElement() {
        ...
        function updateVisual(labelType, season, episodeNum, watched, watchedAt) {
          ...
          if (iconEl) iconEl.style.color = watched ? '#b4339a' : 'rgba(255,255,255,0.38)';
          if (!spanEl) return;
    ...
    function _mdToHtml(md) {
      ...
      function inl(s) {
        return esc(s)
          ...
          .replace(/\[([^\]]+)\]\(([^)]+)\)/g,'<a href="$2" style="color:#b4339a;text-decoration:none">$1</a>');
      }
      var lines = md.split('\n');
