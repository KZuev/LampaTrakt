...
| v3.2.36 | TBD | Фикс подсчёта просмотренных серий (Мои сериалы/галочки/остаток): считать серию просмотренной по `plays>0 ИЛИ last_watched_at ИЛИ completed` (Trakt перестал отдавать `plays` под `extended=full` → `watchedEps=0`). 3 места: `watching`, `watchingCounts`, `ensureWatchedCache` |
