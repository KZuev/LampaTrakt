...
**v2.9.11** — последний релиз, ждёт проверки.
...
| v2.9.11 | TBD       | «Смотрю сериалы»: фильтр скрытых/дропнутых чинился — эндпоинт `/users/me/hidden/*` не существует (404 → пустой набор), исправлен на `/users/hidden/{section}`, добавлена секция `dropped`, убрана несуществующая `watched` |
