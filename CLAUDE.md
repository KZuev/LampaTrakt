...
**v3.2.13** — «Открыть случайный» в «Хочу посмотреть»: кнопка добавлена в меню сортировки `watchlistHub` (через `topAction`). Хаб захватывает текущий набор результатов через `onData`-коллбэк и при выборе открывает случайный элемент через `Lampa.Activity.push`.
