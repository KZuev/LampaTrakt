...
**v3.1.4** — Реалтайм-статус на странице описания: после своей отметки `S.. · E..` / «Просмотрено» обновляется на месте через `_fullCardRefreshFn` (без переинсерта и сброса фокуса).
