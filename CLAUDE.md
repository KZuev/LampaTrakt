...
Настоящий бренд — градиент из логотипа Trakt: `#9f42c6 → #b4339a → #cf2061 → #f50613` (фиолетовый→красный). **Не вводить заново** `#c850c0` или `#9b59d0`/`rgba(155,89,208,…)` — это были имитации, удалены в v3.2.24.

| Значение | Где / почему |
|---|---|
| `linear-gradient(135deg,#9f42c6,#b4339a,#cf2061,#f50613)` | Фоновые акценты бренда: бейджи на постерах (просмотрено/хочу/история/цифровой релиз), `.trakt-progress-bar__fill`. Применять только к `background` |
| `linear-gradient(180deg,#9f42c6,#f50613)` | Вертикальная полоска-разделитель `.trakt-upcoming-section::before` |
| `#b4339a` (середина градиента) | Сплошной бренд там, где градиент не применить: значок в шапке (`.trakt-head-action`/`.trakt-account-switcher` — именно `color:`, чтобы бел./фокус v3.2.20/21 работали), цифра бейджа аккаунта, текст статуса «Смотрю», `border-bottom` активных вкладок/сортировок, ссылки README-рендера, кнопка «В списке» |
| `rgba(180,51,154,α)` (тот же тон) | Тинты выделения: фон `.18` и inset-тень `.3` активных вкладок/сортировок, hover статуса; двухтоновый полукруг бейджа «Смотрю» (`.9`/`.52`) |
