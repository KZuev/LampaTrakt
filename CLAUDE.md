...
**v3.0.28** — Фикс: после сброса токена через бэкап localStorage слот аккаунта тоже очищается, настройки показывают «Не привязан».
...
| v3.0.28 | TBD | Фикс: после недействительного токена (401 на refresh) очищается слот → UI показывает «Не привязан» |
