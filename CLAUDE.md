...
**v3.2.16** — Дата цифрового релиза в карточке: полная дата `DD.MM.YYYY` (как было раньше) если цифровой релиз известен; «Скоро» — только для `soonMovieIds` (как бейдж «СКОРО» на постере, цифровой даты нет). В отличие от бейджа на постере карточка НЕ использует короткий относительный формат («Завтра» / «15 июн»).
...
| v3.2.16 | TBD | Карточка: полная дата `DD.MM.YYYY` если цифровой релиз известен (как раньше), «Скоро» только для `soonMovieIds`. Без короткого относительного формата постера |
