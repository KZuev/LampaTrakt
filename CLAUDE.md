...
**v2.9.13** — последний релиз, ждёт проверки.
...
| v2.9.13 | TBD       | (1) История: серия в названии — «Футурама (S01E04)» (суффикс после `enrichWithTmdbLocale`, иначе затирается); (2) «Мои сериалы»: фикс навигации вверх на табы (`onHead` в `baseComponent` работал только для `watchlist`, добавлен `watching`); (3) уменьшен шрифт табов + ellipsis (`.trakt-watching-hub__tabs`) |
